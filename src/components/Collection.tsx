    ...
    async function fetchSets() {
      try {
        ...
        if (!isStaticHosting) {
          ...
          if (res.ok) {
            fetchedData = await res.ok ? await res.json() : null;
          }
        }
        
        if (!fetchedData) {
          // Fallback to calling the official TCG API directly
        ...
        if (fetchedData && fetchedData.length > 0) {
          ...
          throw new Error('Fallback to local list');
        }
      } catch (err) {
        console.error('Error fetching sets, using local list:', err);
        setSets([
  ...
  const handleDatabaseSearch = async (e?: React.FormEvent, pageToLoad: number = 1) => {
    ...
    try {
      ...
      if (!fetchedData) {
        ...
        if (selectedSet) {
          if (qString) qString += ' ';
          qString += `set.id:${selectedSet}`;
        }
        const encodedQuery = encodeURIComponent(qString);
        const url = qString 
          ? `https://api.pokemontcg.io/v2/cards?q=${encodedQuery}&page=${pageToLoad}&pageSize=60&orderBy=name`
          : `https://api.pokemontcg.io/v2/cards?page=${pageToLoad}&pageSize=60&orderBy=name`;
          
        const res = await fetch(url);
        if (res.ok) {
          const apiRes = await res.json();
          // Format sets exactly like our backend formats them
          const formattedCards = (apiRes.data || []).map((card: any) => ({
            id: card.id,
            name: card.name,
            images: { small: card.images?.small || card.images?.large || '', large: card.images?.large || card.images?.small || '' },
      ...
      if (fetchedData) {
        const formatted = (fetchedData.data || []).map((card: any) => ({
          id: card.id,
          name: card.name,
          imageUrl: card.images?.small || card.images?.large || card.imageUrl || '',
          setCode: card.set?.id || card.setCode || '',
          setName: card.set?.name || card.setName || '',
          setNumber: card.number || card.setNumber || ''
        }));
